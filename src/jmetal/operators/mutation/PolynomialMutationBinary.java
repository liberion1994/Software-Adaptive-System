 ...
 */
public class PolynomialMutationBinary extends Mutation {
  ...
  private static List VALID_TYPES = Arrays.asList(IntSolutionType.class) ;
	 ...
	 */
	public PolynomialMutationBinary(HashMap<String, Object> parameters) {
		super(parameters) ;
	...
	public void doMutation(double probability, Solution solution) throws JMException {
		...
		for (int var=0; var < solution.numberOfVariables(); var++) {
			if (PseudoRandom.randDouble() <= probability)
			{
				...
				{
					xy = 1.0-delta2;
					val = 2.0*(1.0-rnd)+2.0*(rnd-0.5)*(Math.pow(xy,(eta_m_+1.0)));
