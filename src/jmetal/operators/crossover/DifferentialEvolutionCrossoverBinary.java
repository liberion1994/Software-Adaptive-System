 ...
 * random one. The implementation of both variants are the same, due to that the
 ...
 */
public class DifferentialEvolutionCrossoverBinary extends Crossover {
	/**
	...
	private static final String DEFAULT_DE_VARIANT = "rand/1/binary";

	/**
	 * Valid solution types to apply this operator
	 */
	private static List VALID_TYPES = Arrays.asList(IntSolutionType.class);
	 ...
	 * Constructor
	 */
	public DifferentialEvolutionCrossoverBinary(HashMap<String, Object> parameters) {
		super(parameters);
...
//		}
		...
		if ((DE_Variant_.compareTo("rand/1/bin") == 0)
			...
			for (int j = 0; j < numberOfVariables; j++) {
				if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
					int value;
					value = (int) (xParent2.getValue(j) + F_
							* (xParent0.getValue(j) - xParent1.getValue(j)));

					if (value < xChild.getLowerBound(j) || value > xChild.getUpperBound(j)) {
						value = (int) (xChild.getLowerBound(j) + PseudoRandom.randDouble(0, 1)
								* (xChild.getUpperBound(j) - xChild.getLowerBound(j)));
					}
	...
	public Object execute_5(Object object) throws JMException {
		...
		if (!(VALID_TYPES.contains(parent[0].getType().getClass())
			...
			Configuration.logger_
					.severe("DifferentialEvolutionCrossover.execute: "
							...
							+ parent[0].getType() + " and "
							+ parent[1].getType() + " and "
							+ parent[2].getType() + " and "
							+ parent[3].getType() + " and "
							+ parent[4].getType() + " are obtained");
		...
		// STEP 4. Checking the DE variant
		if (DE_Variant_.compareTo("DE/rand/2/bin") == 0) {
			for (int j = 0; j < numberOfVariables; j++) {
				if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
					double value;
					value = xParent4.getValue(j) + F_
							* (xParent0.getValue(j) - xParent1.getValue(j))
							+ F_
		...
		} else if (DE_Variant_.compareTo("DE/rand-to-best/bin") == 0) {
			for (int j = 0; j < numberOfVariables; j++) {
				if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
					double value;
					value = xParent4.getValue(j) + F_
	...
	public Object execute_6(Object object) throws JMException {
		...
		if (!(VALID_TYPES.contains(parent[0].getType().getClass())
			...
			Configuration.logger_
					.severe("DifferentialEvolutionCrossover.execute: "
							...
							+ parent[2].getType() + " and "
							+ parent[3].getType() + " and "
							+ parent[4].getType() + " and "
							+ parent[5].getType() + " are obtained");
		...
		XReal xParent3 = new XReal(parent[3]);
		XReal xParent4 = new XReal(parent[4]);
		XReal xParent5 = new XReal(parent[5]);
		XReal xCurrent = new XReal(current);
		XReal xChild = new XReal(child);
		...
		// STEP 4. Checking the DE variant
		if (DE_Variant_.compareTo("DE/rand-to-best/2/bin") == 0) {
			for (int j = 0; j < numberOfVariables; j++) {
				if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
					double value;
					value = xParent5.getValue(j) + K_
							* (xParent4.getValue(j) - xParent5.getValue(j))
							+ F_
							* (xParent0.getValue(j) - xParent1.getValue(j))
							+ F_
							* (xParent2.getValue(j) - xParent3.getValue(j));
		...
		else if (DE_Variant_.compareTo("DE/current-to-rand/2/bin") == 0) {
			for (int j = 0; j < numberOfVariables; j++) {
				if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
					double value;
					value = xParent5.getValue(j) + K_
